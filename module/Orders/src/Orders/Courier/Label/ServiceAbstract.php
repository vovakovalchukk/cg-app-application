...
abstract class ServiceAbstract implements LoggerAwareInterface
{
    ...
    protected function persistProductDetailsForOrders(
        ...
        foreach ($suitableOrders as $order) {
            ...
            $parcelsData = ($orderParcelsData->containsId($order->getId()) ? $orderParcelsData->getById($order->getId()) : $this->getDummyParcelDataForOrder($order));
            /** @var OrderParcelsData $parcelsData */
    ...
    protected function getDummyParcelDataForOrder(Order $order): OrderParcelsData
    {
