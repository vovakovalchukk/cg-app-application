...
//            /* @var $rmDeliveryService \CG\Intersoft\RoyalMail\DeliveryService\Service */
//            $rmDeliveryService = $di->newInstance(CG\Intersoft\RoyalMail\DeliveryService\Service::class);
//
//            $rmDeliveryService->getDeliveryServices();

            /* @var $implService \CG\CourierAdapter\Provider\Implementation\Service */
            $implService = $di->newInstance(CG\CourierAdapter\Provider\Implementation\Service::class);

            /** @var \CG\Account\Client\Service $accountService */
            $accountService = $di->newInstance(CG\Account\Client\Service::class);

            $account = $accountService->fetch(7);

            $courierInstance = $implService->getAdapterImplementationCourierInstanceForAccount($account);

            echo get_class($courierInstance)."\n";
...
//            print_r($deliveryServices);
            ...
            foreach ($deliveryServices as $deliveryService) {
                ...
                echo "isDomestic ".$shipmentClass::isDomestic()."\n";
